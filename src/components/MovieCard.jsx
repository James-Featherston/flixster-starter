  ...
  return (
    <>
      <article onClick={() => setModal(!modal)} id="movie-card">
        <img id="movie-img" src={props.movie.poster === null ? movieImg : props.movie.poster} alt={props.movie.title} />
        <h2>{props.movie.title}</h2>
        <p>Rating: {props.movie.rating}</p>
        <div className='movie-buttons'>
          {
            watched ? 
            <span onClick={handleWatchClick} className="material-symbols-outlined">
              visibility
            </span>
            :
            <span onClick={handleWatchClick} className="material-symbols-outlined">
          ...
          {
            liked ? 
            <span onClick={handleLikedClick} className="material-symbols-outlined red_btn">
              favorite
            </span>
            :
            <span onClick={handleLikedClick} className="material-symbols-outlined">
              favorite
            </span>
          }
        </div>
